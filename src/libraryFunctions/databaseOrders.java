...
public class databaseOrders {
    ...
    public static void addUser(userDetailsObject newUser) {
        try {

            Connection con = DriverManager.getConnection("jdbc:derby://localhost:1527/StadiumBookingSystemNea", "Noah", "password");
            Statement statement = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);

            String sql = "INSERT INTO USERTABLE (userID, email, password, firstname, lastname, dateofbirth) VALUES (" + newUser.getUserID() + ", " + newUser.getEmail() + ", " + newUser.getPassword() + ", " + newUser.getFirstName() + ", " + newUser.getLastName() + ", " + newUser.getDateOfBirth() + ")";
            ResultSet rs = databaseManagement.executeQuery(sql);
